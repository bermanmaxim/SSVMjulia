...
# uncomment to disable parallel computations:
# pmap = Base.map
...
"""
function ssvm1slack(param, options, helper=Dict())
    time0 = time()
    ...
    helper[:test_iter] = Int[]
    helper[:w_serie] = Vector{Float64}[]
    ...
    if quadratic == :primalqp
        ...
        for idx in positivity
            problem.constraints += 1000000000 * W[idx] >= positivity_zero
        end
    end
    if quadratic == :pegasos
        verbose >= 3 && println("[SSCM1slack] adding positivity constraints in A")
        for idx in positivity
            vec = zeros(N)
            vec[idx] = 10000000000
            push!(A, vec)
            push!(b, positivity_zero)
        end
    end
    ...
    test_iter = (Ntest > 0) ? 0 : Inf # next testing iteration (start by a test)
    ...
    while (!converged)
        ...
        if cachesize > 0 && lencache(cache) >= mincachesize
        ...
        if !cache_updated || (cache_updated && (violation < eps))
        # if (cachesize > 0 && lencache(cache) < mincachesize) || violation < eps
            ...
            if 0 < oldn < n
                ...
                if quadratic == :primalqp
                    ...
                    for idx in positivity
                        newproblem.constraints += W[idx] >= positivity_zero
                    end
        ...
        if (n == N) && (iter >= max_iter)
            ...
            verbose >= 1 && print("[SSCM1slack] Finished (atteigned max_iter)!")
            break
        end
        ...
        if converged
            # we have converged
            verbose >= 1 && print("[SSCM1slack] Finished (converged)!")
            break
        end       	
    end
    println()
